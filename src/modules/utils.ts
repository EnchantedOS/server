...
export function useUtil(rconConnection: RconConnection) {
  if (rconConnection.config?.util?.initialized) return
      ...
      default: {
        async isOp(player) {
          ...
          if (typeof player !== 'string')
            throw new Error('util.isOp: Player name needs to be a string')

          return readFile(
            join(
              process.cwd(),
              rconConnection.config.javaServer.path,
              'ops.json'
            )
          )
            .catch(() => {
            ...
            .then(
              ops =>
                !!ops.filter((op: { name: string }) => op.name === player)
                  .length
            )
        },
        ...
        async isOnline(player) {
          if (!player)
            throw new Error('util.isOnline: Please provide player name')
          if (typeof player !== 'string')
            throw new Error('util.isOnline: Player name needs to be a string')

          const result = await rconConnection.send(
        ...
        async getOnline() {
          ...
          if (!online)
            throw new Error(
              'util.getOnline: Could not parse list command result'
            )

          return {
            online: parseInt(online[1]),
        ...
        async tellRaw(message, target = '@a', options = {}) {
          ...
          if (typeof options !== 'object')
            throw new Error(
              'util.tellRaw: Options for tellraw should be an object'
            )
        ...
        async getEntityData(target = '@a', path = '', scale) {
          ...
          if (path && scale !== undefined) cmd += ` ${scale}`

          const rawResult = await rconConnection.send(cmd)
          if (rawResult.match(/^No entity was found$/))
            throw new Error(`util.getEntityData: Invalid target ${target}`)
        ...
        async getDimension(player) {
          ...
          if (!dimension)
            throw new Error(
              'util.getLocation: Could not parse dimension, player may be offline'
            )
        ...
        async getLocation(player) {
          if (!player)
            throw new Error('util.getLocation: Please provide player name')
          if (typeof player !== 'string')
            throw new Error(
              'util.getLocation: Player name needs to be a string'
            )
          ...
          if (!location)
            throw new Error(
              'util.getLocation: Could not parse location, player may be offline'
            )

          return {
            x: parseInt(location[1]),
            y: parseInt(location[2]),
            z: parseInt(location[3]),
        ...
        async teleport(target, location) {
          ...
          if (typeof target !== 'string')
            throw new Error('util.teleport: Target needs to be a string')
          if (
            !location ||
          ...
          await rconConnection.send(
            `execute in ${location.dimension} as ${target} at @s run playsound entity.item.pickup master @a ${location.x} ${location.y} ${location.z} 10 1 1`
          )
        },
